...
//            /** @var PHPUnit_Framework_MockObject_MockObject|AbstractTransferEvent $event */
//            $event = $this->getMockForAbstractClass(AbstractTransferEvent::class,[],"",false,false,true,[]);
//            $event->expects($this->once())->method("getResponse")->willReturn($resp);

            $excMsg = "";
            try {
                $res = $request->parseSuggests($data["input"]);
//                echo "[\"".implode('", "',$res)."\"]"."\n";
            } catch (Exception $e) {
                $res = get_class($e);
